...
type Row={uid:string;nickname?:string;state:'signed'|'already'|'failed';at:string;credit?:number;balance?:number;error?:string}
...
function App(){
  ...
  const refresh=async(notify=false,path='/buddy-checkin/status',init?:RequestInit)=>{try{const response=await fetch(path,init);if(!response.ok)throw new Error(`HTTP ${response.status}`);const x=await response.json() as unknown;const d=x as Data;if(!d||typeof d!=='object'||!Array.isArray(d.results))throw new Error('unexpected status shape');setData(d);if(notify&&d.notice)setToast(d.notice)}catch{setData(previous=>previous??{status:'error',results:[]})}}
  ...
  const retry=async()=>{setRetrying(true);try{const response=await fetch('/buddy-checkin/status',{method:'POST'});if(!response.ok)throw new Error(`HTTP ${response.status}`);setData(await response.json() as Data)}finally{setRetrying(false)}}
  ...
  return <>
    ...
    {open&&<div style={{position:'fixed',zIndex:1000,width:310,right:20,top:72,background:bgLayer,border:`1px solid ${borderL2}`,borderRadius:10,padding:12,boxShadow:'0 18px 45px #0008',color:labelPrimary,fontSize:13}}>
      ...
      <div style={{color:labelSecondary,fontSize:12,margin:'7px 0'}}>本次启动检查：{data.checkedAt?new Date(data.checkedAt).toLocaleString():'—'}{data.balanceCheckedAt&&<> · 余额更新：{new Date(data.balanceCheckedAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</>}</div>
      ...
      <div style={{maxHeight:120,overflowY:'auto'}}>
        {data.results.map(row=><div key={row.uid} style={{padding:'7px 0',borderTop:`1px solid ${borderL2}`}}>
          ...
          <div style={{fontSize:12,color:labelSecondary,marginTop:3}}>{row.balance===undefined?'积分：暂不可用':`剩余积分：${row.balance.toLocaleString()}`}</div>
        </div>)}
      </div>
