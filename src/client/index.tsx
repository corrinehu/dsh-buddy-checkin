...
type Data={checkedAt?:string;notice?:string;status:'none'|'ok'|'warn'|'error';results:Row[]}
...
function App(){
  ...
  useEffect(()=>{void fetch('/buddy-checkin/status').then(r=>{if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json()}).then((x:unknown)=>{const d=x as Data;if(!d||typeof d!=='object'||!Array.isArray(d.results))throw new Error('unexpected status shape');setData(d);if(d.notice)setToast(d.notice)}).catch(()=>setData({status:'error',results:[]}))},[])
