window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function App() {
			...
			const refresh = async (notify = false, path = "/buddy-checkin/status", init) => {
				...
				try {
					...
					setData(d);
					if (notify && d.notice) setToast(d.notice);
				} catch {
					if (id !== requestId.current) return;
			...
			const retry = async () => {
				setRetrying(true);
				try {
					await refresh(false, "/buddy-checkin/status", { method: "POST" });
				} finally {
					setRetrying(false);
				}
			};
			...
			return /* @__PURE__ */ react.createElement(react.Fragment, null, toast && /* @__PURE__ */ react.createElement("div", { style: {
				...
				onClick: () => {
					setOpen(true);
					refresh(false, "/buddy-checkin/refresh-balance", { method: "POST" });
				},
				title: "WorkBuddy 签到",
				style: {
					display: "inline-flex",
				...
				fontSize: 12,
				margin: "7px 0"
			} }, "本次启动检查：", data.checkedAt ? new Date(data.checkedAt).toLocaleString() : "—"), loading && /* @__PURE__ */ react.createElement("div", { role: "status" }, "正在查询…"), queryError && /* @__PURE__ */ react.createElement("div", { role: "alert" }, queryError), /* @__PURE__ */ react.createElement("div", { style: {
				maxHeight: 120,
				overflowY: "auto"
				...
				color: labelSecondary,
				marginTop: 3
			} }, row.balance === void 0 ? "积分：暂不可用" : `剩余积分：${row.balance.toLocaleString()}`, row.balanceError ? ` · ${row.balanceError}` : row.balanceCheckedAt ? ` · ${new Date(row.balanceCheckedAt).toLocaleTimeString()} 更新` : " · 上次记录")))), failed && /* @__PURE__ */ react.createElement("button", {
				disabled: retrying,
				onClick: () => void retry(),
				style: {
					marginTop: 8,
